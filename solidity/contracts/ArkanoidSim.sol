...
contract ArkanoidSim {
    ...
    function initializeGame(int _paddleX, int _ballSpeedX, int _ballSpeedY,uint _maxSteps) public {
        ...
        state.paddleSpeed = PADDLE_SPEED;
        maxSteps = _maxSteps;
    ...
    function updateGame() public {
        ...
        if (state.ballY + int(BALL_RADIUS) >= state.paddleY 
            ...
            state.ballY = state.paddleY - int(BALL_RADIUS);
            state.hits += 1;
        }
