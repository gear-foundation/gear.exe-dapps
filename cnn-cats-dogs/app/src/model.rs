...
impl DenseLayer {
    pub fn new(filters: Vec<Vec<i64>>, bias: Vec<i64>) -> Self {
        let filters: Array2<i128> = Array2::from_shape_vec(
            ...
            filters.into_iter().flatten().map(|x| x as i128).collect(),
        )
...
impl Model {
    ...
    pub fn dense_1_apply(&self, input: &Array2<i128>) -> Array1<i128> {
        ...
        for idx in 0..result.len() {
            ...
            let scaled_value = normalized.checked_mul(gamma_c).expect("Scaling overflow") >> SHIFT;
            let final_value = scaled_value.checked_add(beta_c).expect("Addition overflow");
...
fn multiply_fixed_point_matrices(a: &Array2<i128>, b: &Array2<i128>) -> Array2<i128> {
    assert_eq!(a.shape()[1], b.shape()[0], "Wrong matrix size");
...
fn convert_to_array2<const M: usize, const N: usize>(array: [[i128; M]; N]) -> Array2<i128> {
